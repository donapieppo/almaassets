namespace :almaassets do
  ...
  task assign_categories: :environment do
    ...
    Good.where(category_id: nil).find_each do |good|
      if good.unibo_description =~ /SEDIA (FISSA|PIEGHEVOLE|GIREVOLE|OPERATIVA|SETPOINT|CON SEDUTA|ERGONOMICA)/i 
      ...
      elsif good.unibo_description =~ /SCRIVANI|ARMADIO|CASSETTIER|ANTA|APPENDIABITI|ARMADIATURE|ARMADIETTO|CATTEDRA|LAMPADA|LAVAGNA|LIBRERIA|POLTRONCIN|POLTRONA|SEGGIOLA|TAVOLINO|TAVOLO|PANCA IN LEGNO|SEDIA|TENDA|VENTILATORE/i
        good.update_attribute(:category_id, chair_id)
      ...
      elsif good.unibo_description =~ /PC DELL|WORKSTATION|PC\s+DESKTOP|PC OLIDATA|PERSONAL COMPUTER|PC OPTIPLEX|PC ASSEMBLATO|PC MAXDATA|COMPUTER ASSEMBLATO|ELAB. IMAC 2|IMAC 27/i 
        ...
        good.update_attribute(:category_id, chair_id)
      elsif good.unibo_description =~ /PC\s+PORTATILE|NOTEBOOK|MACBOOK AIR|PC SLIM CASE|MACBOOK 12/i 
